import colors from './colors';
import { ThemePalette } from '../types';

const themeColors = {
  neutral: {
    0: '#FFFFFF',
    10: '#F2F2F2',
    15: '#E6E6E6',
    20: '#D9D9D9',
    30: '#CCCCCC',
    40: '#B3B3B3',
    50: '#999999',
    60: '#808080',
    70: '#666666',
    75: '#515151',
    80: '#4D4D4D',
    85: '#333333',
    90: '#1A1A1A',
    95: '#0A0A0A',
    100: '#000000',
  },
  primary: {
    main: '#39006e',
    mainHover: '#62007A',
    mainActive: '#6F0881',
    surface: '#FFF0FD',
    surfaceHover: '#F5B6FF',
    surfaceActive: '#F5B6FF',
    border: '#6B0E99',
    borderHover: '#6B0E99',
    borderActive: '#C730E5FF',
    accent: '#ff008c',
  },
  secondary: {
    main: '#111546',
    mainHover: '#0800E8',
    mainActive: '#0D0881',
    surface: '#f7f6ff',
    surfaceHover: '#d9d2fd',
    surfaceActive: '#d9d2fd',
    border: '#030154',
    borderHover: '#030154',
    borderActive: '#1F16C833',
  },
  special: {
    main: '#5B00D1',
    mainHover: '#541e9c',
    mainActive: '#990E5D',
    surface: '#F0E5FF',
    surfaceHover: '#eca9f1',
    surfaceActive: '#eca9f1',
    border: '#990e97',
    borderHover: '#990e97',
    borderActive: '#eb54ed',
  },
  danger: {
    main: colors.rose['750'],
    mainHover: colors.rose['700'],
    mainActive: colors.rose['600'],
    surface: colors.rose['50'],
    surfaceHover: colors.rose['100'],
    surfaceActive: colors.rose['50'],
    border: colors.rose['500'],
    borderHover: colors.rose['400'],
    borderActive: colors.rose['300'],
  },
  warning: {
    main: colors.stone['800'],
    mainHover: colors.orange['600'],
    mainActive: colors.yellow['700'],
    surface: colors.yellow['50'],
    surfaceHover: colors.yellow['100'],
    surfaceActive: colors.yellow['50'],
    border: colors.yellow['500'],
    borderHover: colors.yellow['300'],
    borderActive: colors.yellow['200'],
  },
  success: {
    main: colors.green['750'],
    mainHover: colors.green['700'],
    mainActive: colors.green['600'],
    surface: colors.green['50'],
    surfaceHover: colors.green['100'],
    surfaceActive: colors.green['50'],
    border: colors.green['600'],
    borderHover: colors.green['500'],
    borderActive: colors.green['400'],
  },
  info: {
    main: colors.blue['750'],
    mainHover: colors.blue['700'],
    mainActive: colors.blue['600'],
    surface: colors.blue['50'],
    surfaceHover: colors.blue['100'],
    surfaceActive: colors.blue['50'],
    border: colors.blue['600'],
    borderHover: colors.blue['500'],
    borderActive: colors.blue['400'],
  },
} as const;

const shadows = {
  shadow1: '0px 2px 4px 0px rgba(0, 0, 0, 0.16)',
  shadow2: '0px 6px 4px 0px rgba(0, 0, 0, 0.25)',
  shadow3: '0px 8px 8px 0px rgba(0, 0, 0, 0.30)',
};

export const themePalette: ThemePalette = {
  ...shadows,
  ...themeColors,
  mode: 'light',
} as const;
